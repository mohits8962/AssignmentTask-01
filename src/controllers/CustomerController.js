...
const { isValid, isVAlidRequestBody, phoneRegex, emailRegex, objectIdValid} = require('../validators/validator')
...
const createCustomer = async function (req, res) {
    try {
        ...
        if (!isVAlidRequestBody(data)) {
            return res.status(400).send({ status: false, message: "Please give the Input to Create the Customer" })
        }
        ...
        if (!isValid(firstName)) {
            return res.status(400).send({ status: false, message: 'FirstName is mandatory and should have non empty String' })
        }

        if (!isValid(lastName)) {
            return res.status(400).send({ status: false, message: 'LastName is mandatory and should have non empty String' })
        }

        if (!isValid(mobileNumber)) {
            return res.status(400).send({ status: false, message: 'Mobile Number is mandatory and should have non empty String' })
        }
        ...
        if (!isValid(DOB)) {
            return res.status(400).send({ status: false, message: 'DOB is mandatory and should have non empty String' })
        }
        ...
        if (!isValid(address)) {
            return res.status(400).send({ status: false, message: 'Address is mandatory and should have non empty String' })
        }

        // if (!isValid(customerID)) {
        //     return res.status(400).send({ status: false, message: 'customerID is mandatory' })
        // }
        ...
        if (!statusSelection.includes(status)) {
            return res.status(400).send({ status: false, message: `Status should be among  ${statusSelection} ` })
        }
...
const getAllCustomers = async function (req, res) {
    try {
        ...
        return res.status(200).send({ status: true, msg: 'getting list successfully', data: allCustomers })
    }
    catch (err) {
...
const deleteCustomer = async function (req, res) {
    try {
        ...
        if (!objectIdValid(data)) {
            return res.status(400).send({ staus: false, message: "the Customer Id should be in 24 character" })
        }

        const deleteCustomer = await CustomerModel.findByIdAndDelete(data)
        return res.status(200).send({ status: true, message: "customer is deleted successfully" });
    } catch (err) {
